sudo sed -i 's|https://mirrors.ustc.edu.cn/proxmox|http://download.proxmox.com|g' /etc/apt/sources.list.d/ceph.list
sudo sed -i 's|https://mirrors.ustc.edu.cn/proxmox|http://download.proxmox.com|g' /etc/apt/sources.list.d/pve-no-subscription.list
...
git submodule set-url edk2 https://github.com/tianocore/edk2
